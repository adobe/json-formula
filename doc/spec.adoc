...
=== Integers
Some functions accept numeric parameters that are expected to be integers.  In these contexts, if a non-numeric or non-integer value is provided, it will be coerced to a number and then truncated. The specific truncation behaviour rounds down and returns the largest integer less than or equal to the number.
