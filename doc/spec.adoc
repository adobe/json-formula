...
* Operands of the union operator (`~`) that are not already arrays are treated as single-element arrays
...
* A `string` is equal to another `string` if they have the exact same sequence
of code points
...
keys and values. Two JSON objects `x` and `y` are considered equal if they have the same number of key/value pairs and if, for each key value
...
* The comparison of array and objects is a deep comparison.  That is, where nested arrays or objects are found, the nested elements will be included in the comparison.
...
* If operands are mixed types, <<Type Coercion,type coercion>> to number is applied before performing the comparison
...
The union operator (`~`) returns an array formed by concatenating the contents of two arrays. An operand that is not already an array is treated as a single-element array. This applies to all non-array types, including `null` and objects.
