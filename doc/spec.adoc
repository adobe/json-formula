...
Projections allow you to apply an expression to a collection of elements. Projections are created when any form of a <<Bracket Expressions, Bracket Expression>> transforms a source array or when a wildcard is applied to an object:
...
The behavior of chained <<SubExpressions>> or <<BracketExpressions>> are changed to operate on each element of the projection, rather than the array itself.
...
[source%unbreakable]
----
  search(items[*].desc, items) -> ["pens", "pencils", "staplers"]
  search(items[*].desc.upper(@), items) -> ["PENS", "PENCILS", "STAPLERS"]
  search(items[].*, items) -> [
                                ["pens", 3.23],
                                ["pencils", 1.34],
                                ["staplers", 10.79]
                              ]
  search(items[0:2].price * 2, items) -> [6.46, 2.68]
  search(items[?price < 3], items) -> [{"desc": "pencils", "price": 1.34}]
----

A <<Pipe Expressions, pipe expression>> will stop the current projection and process it as a normal array.

For example, if you wanted to sum the `price` values, this expression: `items[*].price.sum(@)` will sum each individual price, returning: `[3.23,1.34,10.79]`. Whereas using a pipe operator results in: `items[*].price | sum(@) -> 15.36`.
...
An `orExpression` will evaluate to either the left expression or the right
...
An `andExpression` will evaluate to either the left expression or the right
...
expression on the left hand side is returned.  This reduces to the
...
childExpr(el-2), ..., childExpr(el-N)]`.  This is a <<Projections, projection>>, and the `childExpr` expression is projected onto the
...
against each individual element in the array (this is also a <<Projections, projection>>).
...
allowed. Implementations should
...
A `STRING` is an expression that allows a literal string value to be
specified.  A literal string is enclosed in double quotes and supports the same character escape sequences as strings in JSON.
...
Number literals follow the same syntax rules as numeric values in JSON with the exception that number literals may omit a leading zero.  For example, `.123` is not valid JSON, but is allowed as a number literal.
...
A bracket with a single signed digit e.g.: `[0]` can be interpreted as a flatten operation or a `multiSelectArray` with the number zero. To resolve this ambiguity, the grammar sets a precedence order so that `[-?[0-9]]` is treated as a an index operation.  To explicitly express an array with one element, use a JSON literal: `` \`[0]` ``
...
The filter expressions are evaluated in the context of the array. For example, given the source JSON:
...
The expression: `partners[@.seniorPartner]` (or `partners[seniorPartner]`) will evaluate to `"Joe"`.
...
This allows functions to be used with <<Projections, projections>>, which can enable functions
...
2. A `pipeExpression` **stops <<Projections,projections>> on the left hand side for
