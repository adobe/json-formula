...
The equality and inequality operators (`=`, `==`, `!=`, `<>`) do **not** perform type coercion.  If operands are different types, the values are not equal.
...
| number | object | empty object: {}
| string | object | create a single-element array with the string.
| boolean | object | create a single-element array with the boolean.
...
TODO: Fix Identifiers in the grammar -- should not include JSON constants
...
If the operands are not the same type, the ordering operators `>, >=, <, <=` will return false.
...
[source%unbreakable]
----
  search(left & "\"" & value & "\"" & right, {"left": "[", "right": "]", "value": "abc" }) -> "[\"abc\"]"
  search(map(&"$" & @, values), {"values": [123.45, 44.32, 99.00] }) -> ["$123.45", "$44.32", "$99"]
----
...
[source%unbreakable]
----
    search([].toNumber(@), ["1", "2", "3", "notanumber", null, true]) -> [1, 2, 3, 0]
----
