...
* Slice and index operations shall coerce operands to a number
...
Coercion is not always possible, and if so, an `invalid-type` <<Errors,error>> shall be emitted.
...
* `ParameterError` is raised when an invalid number of function arguments is
...
In addition to the grammar, there is a token precedence ordered
...
character sequence that does not match the `NAME` grammar rule, it must be enclosed in single quotes.
...
The comparisons follow these rules:
...
From the `bracketExpression` construction, the bracketed contents provide access to the elements in an array or object.
...
When brackets enclose an expression (`'[' expression ']'`), the results of that expression are used to index into an object or array.

When used with an object, the bracketed expression is used as the name of a key in that object. For example, `foo["bar"]` is equivalent to `foo.bar`. If the expression returns a key not present in the object, a `null` value is returned.

Most commonly, index expressions are used with arrays.  With arrays, an expression in brackets is expected to return an integer value to return an offset in the array.
...
An expression is outside the bounds of the array when the value is greater than or equal to the length of the array or less than the negative length of the array.
If an index expression is outside the bounds of the array then a value of `null` is returned.

Index expressions are evaluated in the context of the left hand expression. For example, given the source JSON:
...
The expression: `partners[@.seniorPartner]` (or `partners[seniorPartner]`) will evaluate to `"Joe"`.  Specifically, the nested expression inside: `partners[partners.seniorPartner]` will return `null`.
...
The behavior of <<Chained Expressions>> and <<Bracket Expressions>> are changed to operate on each element of the projection, rather than the array itself.
...
For example, if you wanted to sum the `price` values, this expression: `items[{asterisk}].price.sum(@)` will sum each individual price, returning: `[3.23,1.34,10.79]`. Whereas using a pipe operator results in: `items[*].price | sum(@) -> 15.36`.
...
The `[{asterisk}]` syntax (referred to as an array wildcard expression) is valid for array expressions. The array wildcard will return a <<Projections,projection>> with all
the elements of the source array.  If an array
wildcard expression is applied to any other JSON type, a value of `null` is
returned.

The `.{asterisk}` syntax (referred to as an object wildcard expression) is valid for object expressions.  The object wildcard will return an array
...
A filter expression is defined only for a JSON array.  Attempting
...
does not exist, an `unknown-function` error is raised.
...
is not provided, or too many arguments are provided, then an
`ParameterError` error is raised.
