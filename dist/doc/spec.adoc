...
This document describes the specification for json-formula. In the specification, examples are shown through the use of a `search` function.  The syntax for this function is:
...
The result of applying a json-formula expression against a JSON document will **always** result in valid JSON, provided there are no errors during the evaluation process.  Structured data in, structured data out.

This also means that, with the exception of json-formula expression types,
json-formula only supports the same types supported by JSON:
...
The grammar is specified using Antlr, as described in [here](https://www.antlr.org/)
...
TODO: Fix Identifiers in the grammar
...
A multiselect expression is used to extract a subset of elements from a JSON hash.  There are two version of multiselect, one in which the multiselect expression is enclosed in `{...}` and one which is enclosed in `[...]`.
...
[source%unbreakable]
----
number-literal = (REAL_OR_EXPONENT_NUMBER | SIGNED_INT)
...
A bracket with a single signed digit e.g.: `[0]` can be interpreted as a flatten operation or a multi-select list with the number zero. To handle this ambiguity, the grammar sets a precedence so that `[-?[0-9]]` is consistently treated as a flatten operation.  To explicitly express an array with one element, use a JSON literal: `` \`[0]` ``
...
If the operands are not the same time, the ordering operators `>, >=, <, <=` will return false.
...
* If operands are mixed types, the right hand operand is coerced to the same type as the left hand operand before the comparison is performed
* If the left hand operand is boolean or null, both operands are coerced to a number
...
json-formula assumes that all function arguments operate on the current node unless
the argument is a literal.  Because of this, an
expression such as `@.bar` would be equivalent to just `bar`.
...
[source%unbreakable]
----
    search([].toNumber(@), `["1", "2", "3", "notanumber", null, true]`) -> [1, 2, 3, 1]
----
...
json-formula has various built-in functions that operate on different
